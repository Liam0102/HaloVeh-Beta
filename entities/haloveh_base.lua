...
function ENT:Enter(p)
	...
	if(not self.Inflight) then
		...
		p:SetNWBool("HALO_Wings",self.Wings);
...
function ENT:Exit(kill) --####### Get out @RononDex
	...
	self.Pilot:SetNWBool("HALO_Wings",nil);
...
hook.Add("PlayerHALOitchFlashlight","HALOVehicleSwitchFlashlight",function(p,e)
...
function ENT:ToggleWings()
	if(self.NextUse.Wings < CurTime()) then
		...
		if(IsValid(self.Pilot)) then
			self.Pilot:SetNWBool("HALO_Wings",self.Wings);
		end
...
if CLIENT then
	...
	function ENT:Think()
		...
		if(IsFlying) then
			...
			if(!p.HALO_HideHud) then
				...
				Wings = p:GetNWBool("HALO_Wings") or self:GetNWBool("Wings");
	...
	function HALO_WeaponReticles(self)
	...
	function HALO_HUD_Compass(self,fpvX,fpvY)
		...
		local a = p.HALO_Alt_Alpha or 255;
		tMod = "";
		if(tr.Hit and tr.HitWorld) then
			
			p.HALO_Alt_Alpha = math.Clamp(a + 4,130,255);
		else
			tMod = "+"
			p.HALO_Alt_Alpha = math.Clamp(a - 4,130,255);
		end
	...
	function HALO_HUD_WingsIndicator(class)
		if(LocalPlayer().HALO_HideHud) then return end;
