...
function ENT:Initialize()
	...
	self.ExitModifier = {x = 0, y = 270, z = 0};
    self.CanEject = false;
	self.SeatPos = {
            
		{self:GetPos()+self:GetForward()*-40+self:GetUp()*30+self:GetRight()*-35,self:GetAngles()+Angle(0,90,0)},
		{self:GetPos()+self:GetForward()*-40+self:GetUp()*30+self:GetRight()*35,self:GetAngles()+Angle(0,-90,0)},
		
		{self:GetPos()+self:GetForward()*-34+self:GetUp()*37+self:GetRight()*0,self:GetAngles()+Angle(0,0,0)},
		
		{self:GetPos()+self:GetForward()*40+self:GetUp()*38+self:GetRight()*22,self:GetAngles()+Angle(0,180,0)},
		{self:GetPos()+self:GetForward()*40+self:GetUp()*38+self:GetRight()*-22,self:GetAngles()+Angle(0,180,0)},
	}
	self:SpawnSeats();
...
hook.Add("PlayerLeaveVehicle", "HalofalconSeatExit", function(p,v)
	if(IsValid(p) and IsValid(v)) then
		if(v.IsHalofalconSeat) then
            if(v.HalofalconFrontSeat) then
                local self = v:GetParent();
                p:SetPos(self:GetPos()+self:GetForward()*270+self:GetUp()*170);
            else
                p:SetPos(v:GetPos()+v:GetForward()*30+v:GetUp()*110+v:GetRight()*0);
            end
...
if CLIENT then
	...
	function CalcView()
		...
		if(IsValid(self)) then
			...
			View = SWVehicleView(self,635,185,fpvPos,true);		
			return View;
		end
	end
