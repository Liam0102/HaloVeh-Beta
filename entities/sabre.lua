...
function ENT:Initialize()
	...
	self.WeaponLocations = {
		BottomRight = self:GetPos()+self:GetUp()*115+self:GetRight()*57+self:GetForward()*125,
		TopRight = self:GetPos()+self:GetUp()*115+self:GetRight()*57+self:GetForward()*125,
		BottomLeft = self:GetPos()+self:GetUp()*115+self:GetRight()*-57+self:GetForward()*125,
	...
	self.Cooldown = 2;
	self.CanShoot = true;
	self.Bullet = CreateBulletStructure(70,"unsc");
	self.FireDelay = 0.1;
	self.NextBlast = 1;
            ...
            if(IsValid(self.Pilot)) then 
                if(self.Pilot:KeyDown(IN_ATTACK2) and self.NextUse.FireBlast < CurTime()) then
                    self.BlastPositions = {
						...
						self:GetPos() + self:GetForward() * 35 + self:GetRight() * -90 + self:GetUp() * 90, //2
						self:GetPos() + self:GetForward() * 35 + self:GetRight() * 90 + self:GetUp() * 90, //3
						self:GetPos() + self:GetForward() * 35 + self:GetRight() * -90 + self:GetUp() * 90, //3
                    }
                    self:FireSabreBlast(self.BlastPositions[self.NextBlast], false, 250, 300, true, 8, Sound("weapons/hornet_missle.wav"));
